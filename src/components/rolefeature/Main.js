            ...
            buttons: [
                {
                  ...
                  onClick: () => {
                    deleteGroup({
                        ...
                        {
                            getAllGroupDetails();
                            NotificationManager.success('Group deleted successfully..!','Congratulations!',3000);
    ...
    function selectThisMenu(AccessRightID,FeatureID){
        ...
        updateFeatureAccessRightsUpdate({
            ...
            if(data.status == 200)
            {
                ...
                NotificationManager.success('Feature access updated successfully..!','Congratulations!',3000);
            }
            else if(data.status == 404)
            {
                NotificationManager.error('Feature access not found..!','Error!',3000);
            }else{
                NotificationManager.error('Something went wrong..!','Error!',3000);
            }
        }).catch((error)=>{
...
export function RoleFeatureModalPopup ({setGroupConfig, resetGroupDetails, UserID}){
    ...
    function saveFeaturesData(){
        ...
        // {
        //     NotificationManager.warning('Please enter the group ID!','Warning!',3000);
        // }else 
        if(groupDetails.GroupName == ''){
            NotificationManager.warning('Please enter the group Name!','Warning!',3000);
        }else{
            ...
            addGroup({
                ...
                if(data.status == 200)
                {
                    handleClose();
                    resetGroupDetails();
                    NotificationManager.success('Group added successfully..!','Congratulations!',3000);
                }
                else if(data.status == 404)
                {
            ...
            updateGroup({
                ...
                if(data.status == 200)
                {
                    handleClose();
                    resetGroupDetails();
                    NotificationManager.success('Group updated successfully..!','Congratulations!',3000);
                }
                else if(data.status == 404)
                {
                    NotificationManager.error('Group not found..!','Error!',3000);
                }else{
                    NotificationManager.error('Something went wrong..!','Error!',3000);
                }
            }).catch((error)=>{
        ...
        >
            ...
            <div className="modal-body pb-0">
                <div className="single-input-field">
                    ...
                    <label htmlFor="role-id-input">Group ID</label>
                </div>
                <div className="single-input-field">
