...
export function addCompany({CompanyID,CompanyName,User}){ 
    return fetch(BaseUrl+'/post',{
        method: 'POST',
        headers : requestHeader,
        body: JSON.stringify({"query":"CompanyAdd '"+CompanyID+"','"+CompanyName+"','"+User+"'"})
    })
    .then((response) => {
        return response;
    }) 
...
export function updateCompany({oldCompanyID,newCompanyID,newCompanyName,UserID}){ 
    return fetch(BaseUrl+'/put',{
        method: 'POST',
        headers : requestHeader,
        body: JSON.stringify({"query": "CompanyModify '"+ oldCompanyID +"','"+ newCompanyID +"','"+ newCompanyName +"','"+ UserID +"'"})
    })
    .then((response) => {
        return response;
    }) 
...
export function getLocationInformation(companyID){ 
    // return axios.get(BaseUrl+'/get',{"query": "LocationGet '"+ companyID +"',<ID>"},requestHeader);
    return fetch(BaseUrl+'/get',{
        method: 'POST',
        headers : requestHeader,
...
export function updateLocationInformation({OldLocationID,NewLocationID,NewLocationName,CompanyID,UserID}){ 
    return fetch(BaseUrl+'/put',{
        method: 'POST',
        headers : requestHeader,
        body: JSON.stringify({"query": "LocationModify '"+OldLocationID+"','"+NewLocationID+"','"+NewLocationName+"','"+CompanyID+"','Yes','"+UserID+"'"})
    })
    .then((response) => {
        return response;
    }) 
...
export function getKioskOfflineTotal(){ 
    requestHeader['Authorization'] = `BEARER ${ atob(localStorage.getItem("accessToken") != null ? localStorage.getItem("accessToken") : '') }`;
    return fetch(BaseUrl+'/get',{
        method: 'POST',
        headers : requestHeader,
...
export function getKioskStickerLowTotal(){ 
    return fetch(BaseUrl+'/get',{
        method: 'POST',
        headers : requestHeader,
...
export function getKioskUPSActivatedTotal(){ 
    requestHeader['Authorization'] = `BEARER ${ atob(localStorage.getItem("accessToken") != null ? localStorage.getItem("accessToken") : '') }`;
    return fetch(BaseUrl+'/get',{
        method: 'POST',
        headers : requestHeader,
        body: JSON.stringify({"query":"KioskUPSActiviatedTotalGet ''"})
    })
    .then((response) => {
        if(response?.status != 404 && response?.status != 500){
            return response.json();
...
export function addUpdateKioskInformation({KioskNumber,DeviceNumber,LocationID,Password,ConfigurationID,boxCount,UserID}){ 
    return fetch(BaseUrl+'/post',{
        method: 'POST',
        headers : requestHeader,
        body: JSON.stringify({"query": "KioskAdd '"+KioskNumber+"','"+DeviceNumber+"','"+LocationID+"','"+Password+"','"+ConfigurationID+"','"+boxCount+"','"+UserID+"'"})
    })
    .then((response) => {
        return response;
    }) 
